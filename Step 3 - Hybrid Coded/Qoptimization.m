function bitrate = OptimizizeQ(mov,Q)
...
numberofsets = frames/(pframes+1);            %Number of frame sets (Starting with an I frame and rest is P frames)
frameset = zeros(h,w,pframes+1,numberofsets); %Initialize Frame Set
idx=1;

%Define each frame set
for i=1:(pframes+1):frames-pframes
        frameset(:,:,1:1+pframes,idx) = mov(:,:,:,i:i+pframes);     
        idx=idx+1;
end

%Get the motion vector and the residual by removing temporal redundancies
temporalremoved = zeros(h,w,frames);
totpframes = numberofsets*pframes;  %Total number of P frames in the movie
...
%Define each frame set
for i=1:(pframes+1):frames-pframes
        frameset(:,:,1:1+pframes,idx) = decoded(:,:,:,i:i+pframes);
        idx=idx+1;
end
...
fprintf('Bitrate at %d Quantization level: %d Mbps \n',Q,bitrate);

decoded = uint8(temporaladded);           %Convert to integer         
fprintf('%d Optimization level COMPLETED! \n',Q);