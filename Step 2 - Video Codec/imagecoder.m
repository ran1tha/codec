...
tic;    %Start Stopwatch
%% ENTER QUANTIZATION LEVEL (High Q => Low Quality) | (Or comment out the unneccessary)
...
%plot probability distributions of AC
figure; hist(AC); title('Probability distribution of AC component'); 
xlabel('AC component'); ylabel('Probability'); grid on;
...
for i=1:length(tohuffman)
    for j = 1:length(symbol_g)
        if tohuffman(i)==symbol_g(j)
...
decoded =[]; %decoded matrix
count = 1;

for i = 1:length(fromhuffman)
    compare=[compare,fromhuffman(i)]; %compare matrix concatenate
    
    for j=1:length(symbol_g)
        if length(code{j}) == length(compare) %to compare two arrays their sizes should be equal
        if code{j} == compare  %compare two arrays
            decoded(count) = symbol_g(j); %if there is a match get that symbol
            count=count+1; 
            compare=[];
        end
        end
...
subplot(1,2,2);
filtered = filter2(fspecial('average',4),decoded);      %Image after applying average filter                                                 %4 was selected after examining the psnr
...
%% Compairing Image Sizes before and after compression

before = (h*w*8)/1000000;
fprintf('Bits before compression: %.4f Mbits \n',before);
after = length(encoded)/1000000;
fprintf('Bits after compression: %.4f Mbits \n',after);
ratio = before/after;
